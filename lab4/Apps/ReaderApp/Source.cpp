...
int main() {
	...
	wchar_t readingSemaphoreName[numOfPages][15],
		...
		logfileName[50] = L"log\\readers\\log_",
		processIdString[10];
	int waitTimeMs, freePageIndex, semaphoreAcquisitionResult;

	sharedFileMappingObject = OpenFileMapping(FILE_MAP_READ, FALSE, L"Local\\SharedFile");
	...
	pagesGlobal = MapViewOfFile(sharedFileMappingObject, FILE_MAP_READ, 0, 0, numOfPages * pageSizeBytes);
	...
	for (int i = 0; i < numOfPages; ++i) {
		...
		readingSemaphores[i] = OpenSemaphore(SYNCHRONIZE, FALSE, readingSemaphoreName[i]);
		writingSemaphores[i] = OpenSemaphore(SEMAPHORE_MODIFY_STATE, FALSE, writingSemaphoreName[i]);
	...
	for (int i = 0; i < actionsCount; ++i) {
		...
		freePageIndex = WaitForMultipleObjects(numOfPages, readingSemaphores, FALSE, INFINITE) - WAIT_OBJECT_0;
